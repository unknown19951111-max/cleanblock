...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // DEV ONLY — local smoke test for SHA-256 hash verification.
  // No data is stored or transmitted. Remove before store submission.
  if (message.type === 'DEV_VERIFY_SHA256_TEXT') {
    verifyTextSha256(message.text, message.expectedHex).then((result) => {
      sendResponse(result);
    });
    return true;
  }

  // DEV ONLY — local smoke test for DNR rule-file validation.
  // No data is stored or transmitted. Remove before store submission.
  if (message.type === 'DEV_VALIDATE_RULESET_TEXT') {
    sendResponse(validateRulesetTextAgainstDescriptor(message.text, message.descriptor));
    return true;
  }

  // DEV ONLY — local smoke test for Ed25519 manifest signature verification.
  // No data is stored or transmitted. Remove before store submission.
  if (message.type === 'DEV_VERIFY_MANIFEST_SIGNATURE') {
    verifyManifestSignature(message.manifest).then((result) => {
      sendResponse(result);
    });
    return true;
  }

  // DEV ONLY — local smoke test for full update validation pipeline.
  // No data is stored, applied, or transmitted. Remove before store submission.
  if (message.type === 'DEV_VALIDATE_UPDATE_PACKAGE') {
    validateUpdatePackageLocally({
      manifest: message.manifest,
      filesByPath: message.filesByPath,
      currentRulesetVersion: message.currentRulesetVersion
    }).then((result) => {
      sendResponse(result);
    });
    return true;
  ...
  // DEV ONLY — local smoke test for validated update application.
  // No data is fetched or transmitted. Remove before store submission.
  if (message.type === 'DEV_APPLY_VALIDATED_UPDATE_PACKAGE') {
